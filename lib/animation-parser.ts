 ...
 */
export function parseLinkAnimationDataFromC(cSource: string): { dataArrayName: string; data: Int16Array } | null {
 ...
 */
export function parseLinkAnimationHeaderFromC(cSource: string): { animName: string; frameCount: number; dataArrayName: string } | null {
  // Extract header info
 ...
 */
export function parseActorFrameDataFromC(cSource: string): { dataArrayName: string; data: Int16Array } | null {
  // Extract array name and data
  const arrayMatch = cSource.match(/s16\s+(\w+)\s*\[\s*\]\s*=\s*\{([^}]+)\}/)
  if (!arrayMatch) {
    return null
 ...
 */
export function parseActorJointIndicesFromC(cSource: string): { indicesArrayName: string; indices: JointIndex[] } | null {
  // Extract array name and data
  const arrayMatch = cSource.match(/JointIndex\s+(\w+)\s*\[\s*\]\s*=\s*\{([^}]+)\}/)
  if (!arrayMatch) {
    return null
  }
  ...
  const entryMatches = dataContent.matchAll(/\{\s*(0x[0-9A-Fa-f]+)\s*,\s*(0x[0-9A-Fa-f]+)\s*,\s*(0x[0-9A-Fa-f]+)\s*,?\s*\}/g)
 ...
 */
export function parseActorAnimationHeaderFromC(cSource: string): { animName: string; frameCount: number; frameDataName: string; jointIndicesName: string; staticIndexMax: number } | null {
  // Extract header info: AnimationHeader name = { { frameCount }, frameData, jointIndices, staticIndexMax };
  const headerMatch = cSource.match(/AnimationHeader\s+(\w+)\s*=\s*\{\s*\{\s*(\d+)\s*\}\s*,\s*(\w+)\s*,\s*(\w+)\s*,\s*(\d+)\s*\}/)
  if (!headerMatch) {
    return null
  }
  ...
  return { animName, frameCount, frameDataName, jointIndicesName, staticIndexMax }
}
...
export function parseAnimationFromC(cSource: string): AnimationEntry {
  ...
  const hasActorHeader = /AnimationHeader/.test(cSource) && !/LinkAnimationHeader/.test(cSource)
  ...
  if (hasLinkHeader) {
    ...
    if (dataResult && !headerResult) {
      throw new Error("File contains only Link animation data array - header file needed")
    }

    if (!dataResult && headerResult) {
      throw new Error("File contains only Link animation header - data file needed")
    }
  }
  ...
  if (hasActorHeader) {
    ...
    if (!jointIndicesResult) {
      throw new Error("File contains no joint indices - JointIndex array needed")
    }
 ...
 */
export function combineLinkAnimationFiles(dataSource: string, headerSource: string): LinkAnimationEntry {
 ...
 */
export function combineActorAnimationFiles(frameDataSource: string, jointIndicesSource: string, headerSource: string): ActorAnimationEntry {
  ...
  if (!frameDataResult) {
    throw new Error("Invalid frame data file - could not find s16 array declaration")
  }

  if (!jointIndicesResult) {
    throw new Error("Invalid joint indices file - could not find JointIndex array declaration")
  }

  if (!headerResult) {
    throw new Error("Invalid header file - could not find AnimationHeader declaration")
  }
...
export function combineAnimationFiles(dataSource: string, headerSource: string): AnimationEntry {
  ...
  throw new Error("Actor animations require frame data, joint indices, and header files")
}
...
export function animationDataToBlob(animation: AnimationEntry, dataFileName: string): Blob {
  if (animation.type === "link") {
    ...
    const totalSize = OTR_HEADER_SIZE + 4 + 2 + 4 + rotValuesSize + 4 + jointIndicesSize + 2
    const buffer = new Uint8Array(totalSize)
 ...
 */
export function animationHeaderToBlob(animation: LinkAnimationEntry, dataPath: string): Blob {
