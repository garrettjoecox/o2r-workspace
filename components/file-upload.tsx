		...
		async (file: File) => {
			...
			if (file.name.endsWith(".c") || file.name.endsWith(".h")) {
				...
				reader.onload = (e) => {
					...
					const hasActorJointIndices =
						/JointIndex\s+\w+\s*\[\s*\]\s*=\s*\{/.test(text);
					const hasActorHeader =
						/AnimationHeader\s+\w+\s*=\s*\{/.test(text) &&
						!/LinkAnimationHeader/.test(text);

					if (
						(hasLinkData && hasLinkHeader) ||
						(hasActorFrameData && hasActorJointIndices && hasActorHeader)
					) {
					...
					{needsDataFile ? (
						<>
							<p className="text-lg mb-2 text-foreground">
								Drop the <strong>data file</strong> (.c with s16 array)
							</p>
						...
						<>
							<p className="text-lg mb-2 text-foreground">
								Drop the <strong>header file</strong> (.c with animation header)
							</p>
							<p className="text-sm text-muted-foreground">
								or click to browse
							</p>
						...
						<>
							<p className="text-lg mb-2 text-foreground">
								Drop a{" "}
								<code className="bg-muted px-1.5 py-0.5 rounded d-inline mx-1">
									.c
								</code>{" "}
								animation file
							</p>
							<p className="text-sm text-muted-foreground">
								Supports Link and Actor animations | or click to browse
							</p>
						</>
					)}
				</label>
