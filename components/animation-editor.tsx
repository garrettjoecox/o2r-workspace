...
import { type AnimationEntry, type LinkAnimationEntry, type ActorAnimationEntry, animationToC } from "@/lib/animation-parser"
...
export function AnimationEditor({ animation, onUpdate }: AnimationEditorProps) {
  ...
  useEffect(() => {
    ...
    if (animation.type === "link") {
      ...
      setEditData(Array.from(animation.frameData).map(v => 
        v < 0 ? `-0x${(-v).toString(16).toUpperCase().padStart(4, '0')}` : `0x${v.toString(16).toUpperCase().padStart(4, '0')}`
      ).join(", "))
      setEditJointIndices(animation.jointIndices.map(j => 
    ...
    if (onUpdate) {
      try {
        if (animation.type === "link") {
          ...
          // Parse joint indices
          const jointMatches = editJointIndices.matchAll(/\{\s*(0x[0-9A-Fa-f]+)\s*,\s*(0x[0-9A-Fa-f]+)\s*,\s*(0x[0-9A-Fa-f]+)\s*\}/g)
  ...
  const renderHexDump = () => {
    ...
    const data = animation.type === "link" ? animation.data : animation.frameData
  ...
  const getAnimationInfo = () => {
    if (animation.type === "link") {
      ...
      return {
        ...
        binarySize: animation.frameData.length * 2 + animation.jointIndices.length * 6,
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {info.type} | Frames: {animation.frameCount} | Data Values: {info.dataValues} | Size: {info.binarySize} bytes
          ...
          {view === "edit" ? (
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <Label htmlFor="anim-data">{animation.type === "link" ? "Animation Data" : "Frame Data"} (s16 hex values)</Label>
                  <Textarea
                    id="anim-data"
                    value={editData}
                    onChange={(e) => {
                ...
                {animation.type === "actor" && (
                  <>
                    <div>
                      <Label htmlFor="joint-indices">Joint Indices (JointIndex array)</Label>
                      <Textarea
                        id="joint-indices"
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  {animation.type === "actor" && (
                    <>
                      <div>
                        <span className="text-muted-foreground">Joint Count:</span>
                        <div className="font-mono">{info.jointCount}</div>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Static Index Max:</span>
              ...
              <div>
                <h3 className="font-semibold mb-2">{animation.type === "link" ? "Data" : "Frame Data"} Preview (first 64 values)</h3>
                <pre className="font-mono text-xs whitespace-pre-wrap bg-muted p-4 rounded-md">
                  {Array.from((animation.type === "link" ? animation.data : animation.frameData).slice(0, 64)).map((v, i) => {
                    const hexStr = v < 0 
                      ? `-0x${(-v).toString(16).toUpperCase().padStart(4, '0')}` 
                      : `0x${v.toString(16).toUpperCase().padStart(4, '0')}`
                    return (i % 8 === 0 ? '\n' : '') + hexStr.padStart(7, ' ') + (i < 63 ? ',' : '')
                  }).join(' ')}
                  {(animation.type === "link" ? animation.data.length : animation.frameData.length) > 64 && '\n...'}
                </pre>
              </div>

              {animation.type === "actor" && (
                <div>
                  <h3 className="font-semibold mb-2">Joint Indices Preview (first 10)</h3>
                  <pre className="font-mono text-xs whitespace-pre-wrap bg-muted p-4 rounded-md">
                    {animation.jointIndices.slice(0, 10).map((j, i) => 
                      `{ 0x${j.x.toString(16).toUpperCase().padStart(4, '0')}, 0x${j.y.toString(16).toUpperCase().padStart(4, '0')}, 0x${j.z.toString(16).toUpperCase().padStart(4, '0')} },`
                    ).join('\n')}
